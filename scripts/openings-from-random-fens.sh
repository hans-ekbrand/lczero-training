...
number_of_pieces=$2
let number_of_required_intermediate_lines=$1*10
...
## invoked with "false"
if [[ ! -L $latest_net ]]; then
  rm *.gz ## remove obsolete link
  ln -s $latest_net .
fi
python3 ~/q-from-fens.py -i ${number_of_pieces}-men-fens.pgn > evaluated-${number_of_pieces}-men-fens.txt
## creates evaluated-${number_of_pieces}-men-fens.txt

## select
~/pick-interesting-fens.R $number_of_pieces 0.2
